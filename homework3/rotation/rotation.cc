...
Eigen::Vector3d ToRollPitchYaw(Eigen::Matrix3d rotation) {
	...
	// rev.z()=atan2(-rotation(1,2),rotation(2,2));
  	//rev=rotation.eulerAngles(2,1,0);
  	//cout<<rev<<endl;
  	rev.x()=atan2(rotation(1,0),rotation(0,0));
	rev.y()=atan2(-rotation(2,0),sqrt(pow(rotation(2,1),2)+pow(rotation(2,2),2)));
	rev.z()=atan2(rotation(2,1),rotation(2,2));
  	//rev=rotation.eulerAngles(2,1,0);
  	return rev;
}
